...
typedef struct {
	char Ten[20];
	float Trong_Luong, Gia_Tri, Don_Gia, So_Luong;
	int Phuong_An;
}Do_Vat;
...
Do_Vat * ReadFromFile(float *W, int *n){
	...
	int i = 0;
	while (!feof(f)){
		fscanf(f,"%f%f%f",&dsdv[i].Trong_Luong,&dsdv[i].Gia_Tri,&dsdv[i].So_Luong);
...
void Greedy(Do_Vat *dsdv, int n, float W) {
	for (int i=0;i<n;i++) {
		...
		if (dsdv[i].Phuong_An > dsdv[i].So_Luong) dsdv[i].Phuong_An = dsdv[i].So_Luong;
