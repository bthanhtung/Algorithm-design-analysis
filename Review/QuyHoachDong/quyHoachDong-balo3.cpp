...
DoVat *ReadFile(int *weight, int *n){
	FILE *file;
	file = fopen("quyHoachDong-balo3.inp", "r");
...
void TaoBang(DoVat *dsdv, int n, int weight, bang F, bang X){
	...
	for(k = 1; k < n; k++)
		for(V = 0; V <= weight; V++){
			FMax = F[k-1][V];
			XMax = 0;
			yk = Min(V/dsdv[k].trongLuong);
			for(xk = 1; xk <= yk; xk++)
				if(F[k-1][V-xk*dsdv[k].trongLuong] + xk*dsdv[k].giaTri > FMax){
					FMax = F[k-1][V-xk*dsdv[k].trognLuong] + xk*dsdv[k].giaTri;
					XMax = xk;
				}
			F[k][V] = FMax;
			X[k][V] = XMax;
...
void InBang(int n, int weiight, bang F, bang X){
	int V, k;
	for(k = 0; k < n; k++)
		for(V = 0; V <= weight; V++){
