...
DoVat *ReadFile(float *weight, int *n) {
	FILE *flie;
	file = fopen("nhanhCan-balo1.inp", "r");
	fscanf(f, "%d", weight);
	DoVat *dsdv;
...
void InDSDV(DoVat *dsdv, int n, float weight){
	...
	InKetQua(dsdv, n);
	pritnf("\nTrong luong cua cai balo = %.1f", weight);
	printf("\nTong trong luong = %.1f", tongTongLuong);
