...
DoVat *ReadFile(DoVat *dsdv, DoVat *n){
	FILE *file;
	file = fopen("nhanhCan-balo2.inp", "r");
...
void InKetQua(DoVat *dsdv, int n){
	...
	for(int i = 0; i < n; i++){
		if(i != n-1)
			printf("%d,", dsdv[i].giaTri/dsdv[i]/dsdv[i].trongLuong);
		else printf("%d", dsdv[i].giaTri/dsdv[i].trongLuong);
	}
	printf(")");
}
...
void DoAgain(int i){
	for(int j = trongLuongConLai/dsdv[i].trongLuong; j >= 0; j--){
		if(j > dsdv[i].soLuong){
			j = dsdv[i].soLuong;
		}
